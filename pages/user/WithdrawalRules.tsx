...
const RuleCard: React.FC<{ icon: string, title: string, content: string }> = ({ icon, title, content }) => (
    <div className="bg-white p-6 rounded-xl shadow-lg border border-gray-200 flex items-start space-x-4 hover:shadow-cyan-100 hover:border-cyan-200 transition-all duration-300 hover:scale-[1.02]">
    ...
    return (
        <div className="max-w-4xl mx-auto">
             <div className="text-center mb-10">
                <i className="fa-solid fa-gavel text-5xl text-cyan-500 mb-4"></i>
                <h2 className="text-3xl font-bold text-gray-800">Withdrawal Rules / টাকা তোলার নিয়মাবলী</h2>
                <p className="text-gray-500 mt-2">Please read these rules carefully before making a withdrawal request.</p>
            </div>
            <div className="space-y-6">
                {rules.map(rule => (
                    <RuleCard key={rule.title} icon={rule.icon} title={rule.title} content={rule.content} />
                ))}
            </div>
        </div>
    );
